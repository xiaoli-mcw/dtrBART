...
for (i in 1:rep){
    bart1 <- readRDS(file=paste0("~/BART/a3/results/bart/bart2_",i,".RDS"))
...
jpeg(file="~/BART/a3/outputs/nln_cr.jpg", width=480, height=480)
...
stg2plot <- ggplot(bardata)  +
    ...
    geom_line(aes(y=pot.stg2/1.2, x=method, group=1), size=1, color="blue") +
    geom_point(aes(y=pot.stg2/1.2, x=method, color="Stage 2"), size=3, shape=16) +
    geom_line(aes(y=pot.all/1.2, x=method, group=1), size=1, color="darkgreen") +
    geom_point(aes(y=pot.all/1.2, x=method, color="Overall"), size=3, color="darkgreen", shape=16) +
...
stg1plot <- ggplot(bardata)  +
    ...
    geom_line(aes(y=pot.stg1/5.5, x=method, group=1), size=1, color="red") +
    geom_point(aes(y=pot.stg1/5.5, x=method, color="Stage 1"), size=3, shape=16) +
    geom_line(aes(y=pot.all/5.5, x=method, group=1), size=1, color="darkgreen") +
    geom_point(aes(y=pot.all/5.5, x=method, color="Overall"), size=3, color="darkgreen", shape=16) +
...
pdf(file="~/BART/a3/outputs/nln.pdf", width=18, height=9)
